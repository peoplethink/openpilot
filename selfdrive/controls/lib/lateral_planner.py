...
class LateralPlanner:
  ...
  def update(self, sm):
    ...
    if self.use_lanelines and not self.get_dynamic_lane_profile():
      # ── [FIX 1] ll_prob 감소를 레인모드 블록 안으로 이동 ──────────
      # 기존: 조건 밖에서 감소 → 커브 진입 시 차선 신뢰도 낮아져 인코스 수렴
      ...
      # ── [FIX 2] 레인모드 커브 인코스 보정 ────────────────────────
      # openpilot y좌표: 양수=왼쪽, 음수=오른쪽
      # 좌커브(measured_curvature>0): 인코스=왼쪽 → y를 음수 방향으로 보정
      # 우커브(measured_curvature<0): 인코스=오른쪽 → y를 양수 방향으로 보정
      curve_abs = abs(measured_curvature)
      if curve_abs > 0.003:
        speed_factor = interp(self.v_ego, [5., 15., 30.], [0.4, 1.0, 1.2])
        correction = interp(curve_abs,
                            [0.003, 0.008, 0.015, 0.025],
                            [0.0,   0.025, 0.055, 0.08]) * speed_factor
        d_path_xyz[:, 1] -= np.sign(measured_curvature) * correction
      # ──────────────────────────────────────────────────────────────
    ...
    # ── [FIX 3] path_offset 부호 수정 ─────────────────────────────
    # UI 설명: 양수(+)=왼쪽, 음수(-)=오른쪽
    # y축 양수=왼쪽이므로 += 가 맞음 (기존 유지)
