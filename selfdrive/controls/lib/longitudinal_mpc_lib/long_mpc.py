...
# [FIX] 고속 컷인 급브레이크 방지 상수
# x_obstacle[0]이 한 프레임에 이 값(m) 이상 줄면 컷인으로 판단해 ramp 발동
# (새 lead 감지뿐 아니라 기존 추종 중 lead 교체도 감지)
OBSTACLE_SUDDEN_DROP_THRESHOLD = 8.0
# 속도별 ramp 감소폭 (m/frame, 20Hz 기준)
# 고속일수록 빠르게 수렴해야 정상 추종도 자연스러움
OBSTACLE_RAMP_BP_MS = [0., 30.*CV.KPH_TO_MS, 80.*CV.KPH_TO_MS, 120.*CV.KPH_TO_MS]
OBSTACLE_RAMP_MAX_V = [2.0, 2.5, 3.5, 4.5]
...
class LongitudinalMpc:
  ...
  def reset(self):
    ...
    # [FIX] 컷인 급브레이크 방지용 상태 변수
    self.prev_x_obstacle   = None   # 이전 프레임 x_obstacle 배열 (N+1,)
    self.prev_x_obstacle_0 = None   # 이전 프레임 x_obstacle[0] 스칼라
    self.prev_lead_status  = False  # 이전 프레임 leadOne.status
    self.cutin_ramp_active = False  # ramp 발동 중 여부
  ...
  def update(self, carstate, radarstate, v_cruise, x, v, a, j):
    ...
    # ── [FIX] 고속 컷인 급브레이크 방지: x_obstacle ramp 적용 ───────────────
    # 기존 문제:
    #   prev_lead_status(False→True) 조건만 체크 →
    #   이미 앞차 추종 중(True→True)에 컷인차가 lead 자리를 빼앗는
    #   고속도로 컷인은 ramp 미발동 → x_obstacle 한 프레임 수십m 점프
    #   → MPC 즉시 최대 제동 → 움찔
    #
    # 수정:
    #   1. 새 lead 감지(False→True): ramp 발동
    #   2. 기존 추종 중에도 x_obstacle[0]이 OBSTACLE_SUDDEN_DROP_THRESHOLD 이상
    #      한 프레임 감소 시 컷인 판단, ramp 발동
    #   3. 속도 비례 ramp 감소폭 (고속일수록 빠르게 수렴)
    #   4. 수렴 완료(오차 < 0.5m)까지 cutin_ramp_active 유지
    #   5. FCW(crash_cnt > 0): ramp 즉시 해제 → 긴급 제동 보장
    min_x_obstacles = np.min(x_obstacles, axis=1)

    ramp_max = float(interp(v_ego, OBSTACLE_RAMP_BP_MS, OBSTACLE_RAMP_MAX_V))

    if self.prev_x_obstacle is not None and self.crash_cnt == 0:
      new_lead    = radarstate.leadOne.status and not self.prev_lead_status
      sudden_drop = (self.prev_x_obstacle_0 is not None and
                     (self.prev_x_obstacle_0 - min_x_obstacles[0]) > OBSTACLE_SUDDEN_DROP_THRESHOLD)

      if new_lead or sudden_drop or self.cutin_ramp_active:
        ramped = np.maximum(min_x_obstacles,
                            self.prev_x_obstacle - ramp_max)
        min_x_obstacles = ramped
        # 목표값 수렴 완료 시 ramp 해제
        self.cutin_ramp_active = bool(
          np.any(ramped - np.min(x_obstacles, axis=1) > 0.5)
        )
      else:
        self.cutin_ramp_active = False
    else:
      self.cutin_ramp_active = False

    self.prev_x_obstacle   = min_x_obstacles.copy()
    self.prev_x_obstacle_0 = float(min_x_obstacles[0])
    self.prev_lead_status  = radarstate.leadOne.status
    # ─────────────────────────────────────────────────────────────────────
    ...
    # [FIX] ramp 적용된 min_x_obstacles 사용
    self.params[:,2] = min_x_obstacles
